...
const InvoiceDetail: React.FC = () => {
  ...
  const computedTotals = useMemo(() => {
    if (!data?.items) return { sub: 0, discount: 0, cgst: 0, sgst: 0, grand: data?.total ?? 0 };
    let sub = 0, discount = 0, cgst = 0, sgst = 0;
  ...
  const downloadPdf = async () => {
    if (!id) return; setDownloadingPdf(true);
    try {
  ...
  const openThermal = async () => {
    if (!id) return; setDownloadingThermal(true);
    try {
      const html = await StaffAPI.invoices.getThermal(String(id));
      const w = window.open('', '_blank');
      if (w) { w.document.write(html); w.document.close(); }
    } catch { /* silent */ } finally { setDownloadingThermal(false); }
  };

  const statusBadge = (st?: string) => {
    if (!st) return <span className="px-2 py-0.5 rounded text-xs bg-gray-300 text-gray-800">—</span>;
  ...
  const balanceBadge = () => {
    ...
    if (paid >= total) return <span className="px-2 py-0.5 rounded text-xs bg-green-600 text-white">Paid in Full</span>;
    if (paid > 0) return <span className="px-2 py-0.5 rounded text-xs bg-amber-500 text-white">Partially Paid</span>;
    return <span className="px-2 py-0.5 rounded text-xs bg-red-600 text-white">Unpaid</span>;
  };
  ...
  const methodBadge = (method?: string) => {
    if (!method) return <span className="text-[10px] px-1 py-0.5 rounded bg-muted">—</span>;
    ...
    return <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${map[m] || 'bg-slate-400 text-white'}`}>{m.toUpperCase()}</span>;
  };

  return (
    <div className="p-4 space-y-6">
      {/* Breadcrumb */}
      <nav className="text-[11px] text-muted-foreground flex gap-1 flex-wrap" aria-label="Breadcrumb">
        <button className="underline hover:text-foreground" onClick={() => navigate('/staff-dashboard')}>Dashboard</button>
        <span>/</span>
        <button className="underline hover:text-foreground" onClick={() => navigate('/staff-dashboard/invoices')}>Invoices</button>
        <span>/</span>
      ...
      <div className="flex flex-wrap gap-3 items-center justify-between">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button size="sm" variant="outline" onClick={fetchInvoice} disabled={loading}><RefreshCcw className="h-4 w-4 mr-1" />Refresh</Button>
          <Button size="sm" variant="outline" onClick={() => setPdfOpen(true)} disabled={loading}><Eye className="h-4 w-4 mr-1" />View PDF</Button>
          <Button size="sm" variant="outline" onClick={downloadPdf} disabled={downloadingPdf || loading}><Download className="h-4 w-4 mr-1" />Download</Button>
          <Button size="sm" variant="outline" onClick={() => setThermalOpen(true)} disabled={loading}><Eye className="h-4 w-4 mr-1" />View Thermal</Button>
          <Button size="sm" variant="outline" onClick={openThermal} disabled={downloadingThermal || loading}><Printer className="h-4 w-4 mr-1" />Quick Thermal</Button>
          <Button size="sm" variant="outline" disabled title="Batch PDFs (coming soon)">Batch PDFs</Button>
          <Button size="sm" variant="outline" disabled title="Batch Thermal (coming soon)">Batch Thermal</Button>
          <Button size="sm" variant="outline" disabled><Mail className="h-4 w-4 mr-1" />Email</Button>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="p-4 space-y-2">
          <h2 className="text-sm font-semibold">Party</h2>
            ...
            {data?.notes && <div className="text-xs pt-2"><span className="font-medium">Notes:</span> {data.notes}</div>}
        </Card>
        <Card className="p-4 space-y-2">
        ...
        <Card className="p-4 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-muted-foreground">Manage invoice lifecycle</span>
          </div>
          ...
          <div className="space-y-2 border rounded-md p-3 bg-muted/30">
            <div className="flex items-center justify-between">
              <h3 className="text-[11px] font-semibold tracking-wide">Update Status</h3>
              {statusMessage && <span className="text-[10px] text-green-600">{statusMessage}</span>}
              {statusError && !statusMessage && <span className="text-[10px] text-red-600">{statusError}</span>}
            </div>
            <div className="flex flex-col gap-2">
              <div className="flex gap-2 items-center">
                ...
                <Input
                  ...
                  disabled={statusUpdating}
                />
                <Button size="sm" variant="outline" disabled={!newStatus || statusUpdating} onClick={async () => {
                  if (!id || !newStatus) return;
                  if (data?.status?.toLowerCase() === 'paid' && newStatus !== 'paid') {
                    const ok = window.confirm('Invoice is paid. Change status anyway?');
                    if (!ok) return;
                  }
                  if (newStatus === 'cancelled' && !statusReason) {
                    const ok = window.confirm('Cancelling without a reason. Continue?');
                    if (!ok) return;
                  }
                  try {
                    setStatusUpdating(true); setStatusError(null); setStatusMessage(null);
                    await StaffAPI.invoices.updateStatus(String(id), { status: newStatus, reason: statusReason || undefined });
                ...
                }}>{statusUpdating ? 'Updating…' : 'Apply'}</Button>
              </div>
            </div>
          </div>
          {/* Add Payment Form */}
          <div className="space-y-2 border rounded-md p-3 bg-muted/30">
            <div className="flex items-center justify-between">
              <h3 className="text-[11px] font-semibold tracking-wide">Add Payment</h3>
              {paymentMessage && <span className="text-[10px] text-green-600">{paymentMessage}</span>}
              {paymentError && !paymentMessage && <span className="text-[10px] text-red-600">{paymentError}</span>}
            </div>
            <div className="flex flex-col gap-2">
              <div className="grid grid-cols-5 gap-2 items-center">
                ...
                <Button size="sm" variant="outline" disabled={addingPayment || !paymentAmount || parseFloat(paymentAmount||'0') <= 0 || parseFloat(paymentAmount||'0') > balance} onClick={async () => {
                  ...
                  try {
                    ...
                    if (newBal === 0 && data?.status?.toLowerCase() !== 'paid') {
                      try { await StaffAPI.invoices.updateStatus(String(id), { status: 'paid', reason: 'Auto-marked after full payment' }); await fetchInvoice(); } catch { /* ignore */ }
                    }
                  } catch (e: any) {
                    setPaymentError(e?.response?.data?.message || 'Failed to add payment');
                  } finally {
                    setAddingPayment(false);
                  }
                }}>{addingPayment ? 'Saving…' : 'Add Payment'}</Button>
                <Button size="sm" variant="ghost" disabled={addingPayment && !paymentAmount} onClick={() => { setPaymentAmount(''); setPaymentNotes(''); setPaymentError(null); setPaymentMessage(null); }}>Reset</Button>
              </div>
              <p className="text-[10px] text-muted-foreground">Amount must be greater than 0 and not exceed the remaining balance.</p>
            </div>
          </div>
        </Card>
      </div>
      ...
      <Card className="p-4 space-y-4">
        <div className="flex items-center justify-between"><h2 className="text-sm font-semibold">Line Items</h2></div>
        {loading ? <div className="text-xs text-muted-foreground">Loading...</div> : !data?.items?.length ? <div className="text-xs text-muted-foreground">No items.</div> : (
      ...
      <Card className="p-4 space-y-4">
        ...
        {loading ? <div className="text-xs text-muted-foreground">Loading...</div> : !data?.payments?.length ? <div className="text-xs text-muted-foreground">No payments yet.</div> : (
          <div className="overflow-x-auto">
            <Table>
      ...
      <Card className="p-4 space-y-3 border border-red-300 bg-red-50">
        ...
        <p className="text-[11px] leading-relaxed text-red-600">Deleting an invoice permanently removes its line items and payment history. This cannot be undone.</p>
        {deleteError && <div className="text-[11px] text-red-700">{deleteError}</div>}
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            onClick={async () => {
              ...
              if (hasPayments) {
                const c1 = window.confirm('This invoice has payment records. Deleting will also remove them. Continue?');
                if (!c1) return;
              }
              if (isPaid) {
                const c2 = window.confirm('Invoice is marked as Paid. Are you sure you want to delete it?');
                if (!c2) return;
              }
              const phrase = window.prompt('Type DELETE to confirm permanent removal of this invoice:');
  ...
  <InvoicePdfViewer invoiceId={String(id)} open={pdfOpen} onOpenChange={setPdfOpen} />
  <InvoiceThermalPrint invoiceId={String(id)} open={thermalOpen} onOpenChange={setThermalOpen} />
    </div>
  );
};
