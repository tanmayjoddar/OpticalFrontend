...
const ProductCreateForm: React.FC<Props> = ({ onCreated }) => {
	...
	const validate = (): boolean => {
		...
		if (!form.basePrice || isNaN(Number(form.basePrice)) || Number(form.basePrice) < 0) e.basePrice = 'Valid base price required';
		if (!form.companyId || isNaN(Number(form.companyId))) e.companyId = 'Company required';
		if (!form.eyewearType) e.eyewearType = 'Type required';
		setErrors(e); return Object.keys(e).length === 0;
	};
	const suggestSku = () => { if (form.name) { const base = form.name.replace(/[^A-Z0-9]/gi,'').toUpperCase().slice(0,6); setForm(f => ({ ...f, sku: base + '-' + Math.random().toString(36).substring(2,5).toUpperCase() })); } };
	const suggestBarcode = () => { setForm(f => ({ ...f, barcode: 'BC' + Date.now().toString().slice(-10) })); };
	const reset = () => { setForm(initialState); setErrors({}); setCreated(null); setServerError(null); };
	const submit = async () => {
		if (!validate()) return;
		try {
		...
		} catch (e:any) { setServerError(e?.response?.data?.message || 'Failed to create product'); }
		finally { setSubmitting(false); }
	};

	function navigateToDetail(id: number) { window.location.href = `/staff-dashboard/inventory/products/${id}`; }
		...
		<div className="space-y-6">
			<Card className="glass-card">
				...
				<CardContent className="space-y-6">
					...
					{created && <Alert><AlertDescription>Product created successfully.</AlertDescription></Alert>}
					<div className="grid gap-4 sm:grid-cols-2">
						<div className="space-y-1"><label className="text-xs font-medium">Name</label><Input value={form.name} onChange={e => setForm({...form,name:e.target.value})} />{errors.name && <p className="text-xs text-red-500">{errors.name}</p>}</div>
						...
						<div className="space-y-1"><label className="text-xs font-medium">Size</label><Input value={form.size} onChange={e => setForm({...form,size:e.target.value})} /></div>
						<div className="space-y-1"><label className="text-xs font-medium">Model</label><Input value={form.model} onChange={e => setForm({...form,model:e.target.value})} /></div>
					</div>
					<Separator />
					<div className="grid gap-4 sm:grid-cols-2">
					...
					<div className="flex justify-end gap-2 pt-2">
						{!created && <Button disabled={submitting} onClick={submit}>{submitting ? 'Creating...' : 'Create Product'}</Button>}
						{created && (<><Button variant="outline" onClick={() => navigateToDetail(created.id)}>View Product</Button><Button variant="outline" onClick={() => reset()}>Create Another</Button><Button onClick={() => window.location.href = '/staff-dashboard/inventory/products'}>Go to Catalog</Button></>)}
					</div>
				</CardContent>
			</Card>
		</div>
